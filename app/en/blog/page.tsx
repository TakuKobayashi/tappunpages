      ...
      <div className="page-wrap">
        <section style={{ padding:"var(--sp12) var(--sp6)" }}>
          <div className="container">
            <h1 className="section-heading" style={{ color:"var(--text-dark)" }}>{t.blog.heading}</h1>
            {posts.length === 0 ? (
              <div style={{ background:"rgba(255,255,255,0.85)",borderRadius:"var(--r-xl)",padding:"var(--sp16)",textAlign:"center",border:"2px dashed var(--border-gray)" }}>
                <span style={{ fontSize:"2rem",display:"block",marginBottom:"var(--sp4)" }}>✍️</span>
                <p style={{ fontFamily:"var(--font-mono)",fontSize:"var(--text-sm)",color:"var(--text-muted)" }}>{t.blog.empty}</p>
              </div>
            ) : (
